...
func TestCheckRefreshesLimitsConcurrency(t *testing.T) {
	...
	refreshCount := maxConcurrentRefresh*2 + 1
	...
	if max := exec.MaxConcurrent(); max > maxConcurrentRefresh {
		t.Fatalf("expected at most %d concurrent refreshes, got %d", maxConcurrentRefresh, max)
	}
}
