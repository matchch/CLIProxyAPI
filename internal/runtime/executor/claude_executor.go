...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	body = applyCloaking(ctx, e.cfg, auth, body, e.Identifier(), baseModel)
...
func injectFakeUserID(payload []byte, provider string, model string) []byte {
	metadata := gjson.GetBytes(payload, "metadata")
	if !metadata.Exists() {
		payload, _ = sjson.SetBytes(payload, "metadata.user_id", cachedUserID(provider, model))
		return payload
	}
...
func applyCloaking(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, payload []byte, provider string, model string) []byte {
	...
	// Inject fake user ID
	payload = injectFakeUserID(payload, provider, model)
